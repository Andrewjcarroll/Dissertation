...
\end{equation}
Here $C_{abcd}$ is the Weyl tensor. The null tetrad is constructed from the future directed unit normal $n^a$ to the spatial hypersurface and an orthonormal spatial triad $r^a$, $\theta^a$, and $\varphi^a$. In the wave zone, these spatial vectors approach the usual radial, polar, and azimuthal directions.
...
\begin{equation}
    ...
    m^a \bar{m}_a = 1,
\end{equation}
with all other tetrad inner products vanishing. In this appendix, $l^a$ denotes the outgoing null tetrad vector, while $\ell$ is reserved for the spherical harmonic mode index.
...
\end{equation}
It satisfies the same index symmetries as the Riemann tensor,
\begin{equation}
    C_{abcd}
    =
    -C_{abdc}
...
\begin{equation}
    ...
    =
    C_{abcd} k^a \bar{m}^b k^c \bar{m}^d .
    \label{eq:app waveex psi4-def-repeat}
\end{equation}
...
\end{equation}
up to the overall sign convention chosen for the null tetrad. The strain can therefore be obtained by integrating $\Psi_4$ twice in time:
\begin{equation}
    h(t)
    =
    \int^t dt'

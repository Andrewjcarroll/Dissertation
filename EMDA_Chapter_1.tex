...
The increasing precision of gravitational wave observations has motivated effort to model compact object dynamics in theories beyond general relativity~\cite{Cardoso2012NRHEP,Corman2024,Afshordi2024BlackHolesInsideOut,Bambi2025Mimickers,Bernard2025GenericEFTGWTests}. Numerical relativity is
...
%{\bf EWH:  Can you put the following metric on a single line?}

\begin{align}
ds^2 &= -\left(1-\frac{2mr\cosh^2\alpha}{\rho^2}\right)dt^2 + \rho^2\left(\frac{dr^2}{\Delta}+d\theta^2\right) +\frac{\Sigma}{\rho^2}\sin^2\theta\, d\varphi^2
...
\kappa &= -2ma \sinh^2\alpha \,
\frac{\cos\theta}{r^2 + a^2 \cos^2\theta}.
\label{Kerr-Sen_axion}
\end{align}
...
\subsection{The equations of motion for EMDA}
\label{sec:eom}
...
Baumgarte1999_BSSN, Baumgarte2010}, supplemented by evolution equations for the electromagnetic, dilaton, and axion fields. 
We also employ puncture gauge conditions~\cite{Campanelli2006,Baker2006}.

For completeness, we present here the full equations of motion and their $3+1$ decomposition.
Starting from the action for EMDA as given in Eq.~\ref{EMDA_action}, we can derive the relevant equations of motion.    
...
%string theory.
These include the Einstein equations:
%{\bf EWH:  For the next, let's break this up on to two lines.}  
...
We now describe the diagnostic quantities used to monitor the quality of the evolutions. In particular, we track the Hamiltonian and momentum constraints together with the electromagnetic divergence constraints throughout the computational domain. These quantities provide a global check on the consistency of the evolution and are used to assess the efficacy of the WAMR refinement parameters.

\subsection{Constraint quantities}
\label{sec:constraint_quantities}

We compute the $L_2$ norms of the Hamiltonian and momentum
...
to avoid unduly weighting the norms by these regions. Representative
...
\caption[Hamiltonian and momentum constraint violations]{
...
\caption[Electromagnetic constraint violations]{
...
The tests done here use the first \(20M\) of a head-on evolution of two equal-mass, charged black holes. The black holes have an initial coordinate separation of $16M$, individual masses of \(m_1=m_2=0.50\), and charge-to-mass ratios \(q_1/m_1=q_2/m_2=0.1\). Both black holes have dimensionless spin \(\chi=0.4\) and are initially at rest. In computing the constraint diagnostics, we again exclude a small neighborhood of each puncture when calculating the norms.
...
\caption[Constraint violation versus wavelet tolerance]{
...
$(\chi=0.4)$. The simulations use wavelet tolerances of
$\varepsilon \in {3\times10^{-5}, 1\times10^{-5}, 3\times10^{-6}}$.
These simulations were performed with a maximum refinement level of
$\ell_{\rm max}=15$. Lowering the wavelet tolerance improves the resolution
of dynamically important regions and reduces the overall constraint violation.
}
...
\caption[Constraint violation versus maximum refinement level]{
The sum of the volume-weighted Hamiltonian and momentum constraint violations over the first $(20M)$ of a head-on evolution of two equal-mass charged black holes with charge-to-mass ratio $(q/m=0.1)$ and dimensionless spin $(\chi=0.4)$ each. The simulations compare maximum refinement levels $\ell_{\rm max} \in {14, 15, 16, 17}$, corresponding to finest grid spacings of $h \in {0.0325, 0.0162, 0.00813, 0.00407}$, respectively. The wavelet tolerance is set to $\varepsilon = 1\times 10^{-5}$. Increasing the maximum refinement level reduces the overall constraint violation by allowing additional grid refinement in under-resolved regions.
}
...
%{\bf EWH:  Fix the next two equations.  Put each on a single line if possible in an aligned environment.} 
\begin{equation}
...
\kappa &= \kappa_1(r_1,\theta_1) + \kappa_2(r_2, \theta_2)
\end{align}
where Eqs.~\eqref{Kerr-Sen_dilaton} and~\eqref{Kerr-Sen_axion}
%{\bf EWH:  Check this equation reference.  For me it was broken.}
give the forms of the dilaton and axion fields, respectively, for a
single Kerr--Sen black hole (albeit in Boyer--Lindquist-type coordinates)
for our current theory parameters $(\alpha_0,\alpha_1)=(1,1)$.
\section{Radiative Quantities}

To analyze the radiation emitted, we compute the Newman--Penrose scalar
$(\Psi_4)$, along with the electromagnetic radiation. We extract their multipolar components at $(r \approx 100M)$. The extraction procedure is described below. A summary of the initial data parameters used in our simulations is provided in Appendix~\ref{sec:appx:sim_tab}.
...
\end{align}
Waveform comparisons in this work primarily use the dominant $(\ell,m)=(2,0)$ mode.
To quantify the total radiation emitted in each field, we also compute the corresponding energy fluxes from the extracted multipolar amplitudes.
...
\caption[Extracted $(2,0)$ gravitational, electromagnetic, and dilaton modes]{The real parts of the $(2,0)$ modes of the gravitational radiation $\Psi_4$, electromagnetic radiation $\Phi_1$ and $\Phi_2$, and dilaton field $\phi$, extracted at $R_{\mathrm{ex}} = 100M$ for a binary with charge-to-mass ratio $q_i/m_i = 0.1$, dimensionless spin $\chi = 0.4$, and an initial coordinate separation of $16M$.}
...
% ---------- FIRST SECTION ----------
\textbf{(a) Kerr--Sen Same-Sign Charge and Spin-Aligned Black Hole Tests}
\vspace{0.4em}
...
$(\chi_1,q_1)$ &
$(\chi_2,q_2)$ &
\textrm{Dilaton $\boldsymbol{\phi}$} &
\textrm{Axion $\boldsymbol{\kappa}$} \\
\midrule

$\chi_1=0.10$, 
$q_1\in\{0.0624,0.125,0.25,0.50\}$ &
$\chi_2=0.10$, 
$q_2\in\{0.0624,0.125,0.25,0.50\}$ &
\cmark & \cmark \\[0.3em]
...
% ---------- SECOND SECTION ----------
\textbf{(b) Collisions with Other Black Holes}
\vspace{0.4em}
...
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X
                                  ...
                                  cc@{}}
\toprule
\textrm{Black Hole 1 $(q,\chi)$} &
\textrm{Black Hole 2 $(q,\chi)$} &
$\boldsymbol{\phi}$ &
$\boldsymbol{\kappa}$ \\
\midrule
...
\caption[Axion and dilaton fields during merger]{Scalar fields on the $z=0$ plane for the case of $\chi = 0.4$ and $q_i/m_i = 0.1$ Kerr--Sen black holes. We show the amplitudes of the axion, $\kappa$ (top four panels), and dilaton, $\phi$ (bottom four panels), fields at the beginning of the evolution (top left), shortly before merger (top right), just after merger (bottom left), and at a later time after merger (bottom right).}
...
\caption[Remnant horizon mass, charge, and spin stability]{
...
\caption[Scalarization of initially bald Kerr--Newman binaries]{Scalar fields on the $z=0$ plane for Kerr--Newman black holes with
...
constraint plots were presented in Sec.~\ref{sec:constraint_quantities}.
These diagnostics show that the Hamiltonian, momentum, and electromagnetic
constraint violations remain controlled over the time scales considered
here, including through merger and ringdown.

We also performed a limited refinement study for a short head on binary
evolution, as described in Sec.~\ref{sec:code_performance}. In this study,
we varied both the wavelet tolerance and the maximum number of refinement
levels. Both are WAMR related parameters that help determine the amount and
extent of refinement. The wavelet tolerance was varied between
$3\times10^{-5}$ and $3\times10^{-6}$, and the maximum refinement level was
varied between 14 and 17, corresponding to grid spacings between
$h_{14}=0.0325$ and $h_{17}=0.00407$. Tightening the refinement criteria

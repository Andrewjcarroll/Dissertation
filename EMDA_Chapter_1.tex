...
Zilhao2014,Zilhao2012,Bozzola2019}. In this work, we adapt some of these techniques to the EMDA system by using an elliptic solver based on a modified version of the two 
charged punctures solver introduced 
in~\cite{Bozzola2019}.
...
\bf{EWH:  Can we update this part with the updates that are in the head-on EMDA paper?  There are changes there that should be pulled into this.} 
...
\end{align}
where Eq.~\ref{Kerr Sen_dilaton} gives the form for a dilaton field for a single Kerr--Sen black hole, albeit in Boyer--Lindquist type coordinates, for our current theory 
parameters of $(\alpha_0,\alpha_1) = (1,1)$.

With the initial gravitational, electromagnetic, and scalar fields specified, we can evolve the system and measure the radiation produced during the merger. We now describe the quantities extracted from the simulations and the procedure used to compute the emitted gravitational and electromagnetic radiation.

...
Figure~\ref{fig:sobolev amr mesh comparison} {\bf EWH:  Can you give the parameters of these simulations? e.g. the field being evolved, resolution (maxdepth), grid size, initial data parameters, etc?}  shows a representative side by side comparison of the adaptive mesh structure produced by the two refinement criteria. The Sobolev informed criterion modifies the local refinement indicator by including derivative information, which changes the distribution of refined regions relative to the baseline wavelet criterion.
\FloatBarrier
\begin{figure}[t]
\centering
